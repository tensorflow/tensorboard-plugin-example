# TensorBoard Plugin Example [![Build Status](https://travis-ci.org/jart/tensorboard_plugin_example.svg?branch=master)](https://travis-ci.org/jart/tensorboard_plugin_example)

This repository demonstrates how to:

1. Write a plugin for TensorBoard
2. Build a custom TensorBoard with that plugin added

First [install Bazel] and [install TensorFlow]. All other build and runtime
dependencies will be automatically downloaded by Bazel while building.  Once
those two things are installed, clone this repository and cd into the directory.

To run a custom-built TensorBoard with "my_plugin" added to the UI:

```sh
bazel build //my_plugin/tensorboard_x
./bazel-bin/my_plugin/tensorboard_x/tensorboard_x --logdir=/tmp/foo
```

## Customization Policy

TensorBoard is distributed under the Apache 2.0 license, but the name itself is
a trademark of Google Inc. When distributing custom builds of TensorBoard, we
recommend that they be branded something other than TensorBoard,
e.g. TensorBoard-X.

We have this policy because we want to make TensorBoard as open as possible
without confusing users. For example, users have been confused in the past when
pip installing a custom tensorboard broke their existing TensorBoard install,
due to changes in behavior and namespace collisions.
...
```sh
bazel build -s //my_plugin/tensorboard
